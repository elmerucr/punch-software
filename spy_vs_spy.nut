...
function do_track(track_no) {
	while (true) {
		foreach (pattern in song.tracks[track_no]) {
			//foreach (note in pattern[0]) {
			for (local j=0; j < song.patterns[ pattern ][0].len(); j++) {
				// iterate through number of notes in pattern
				local note = song.patterns[pattern][0][j]
				local instr = song.instruments[song.patterns[ pattern ][2][j]]
				...
				breakhere(song.patterns[pattern][1][j] - 2)
				// end note
